...
titleString BYTE "Random Integer Generator, Sorter, and Counter - by 
Sean Tyler",13,10,13,10,0
; EC strings.
ec1Message BYTE "**EC1 - Arrays are displayed by column.",13,10,0
ec2Message BYTE "**EC2 - Numbers are generated to a file, then read 
into the array.",13,10,13,10,0
...
"original list, sorts it, calculates and 
displays the median value, displays",13,10,
"the list sorted in ascending order, then
counts and displays the number of",13,10,
"instances of each generated value, 
starting with ",0
goodbyeStr BYTE "Hope you enjoyed your random numbers.  
Goodbye!",13,10,13,10,0
; Display headers.
...
instancesHeader BYTE "Your list of instances of each generated 
number:",13,10,0
...
;
; Preconditions: Filename string set and valid, buffer is type DWORD, array is type
DWORD, max, min, 
;and array size are positive ints.
;
; Postconditions: Array is filled with random values; file generated in program 
directory with array values. 
;  No register change.
;
; Receives: 
...
CALL WriteToFile
; Open file.
MOV  EAX, [EBP + 48] ; filename, openinputfile sets EAX as 
filehandle for readfromfile.
CALL OpenInputFile
; Read from file.
...
; Name: displayMedian
;
; Description:  Displays the median of the array of numbers.  If even, displays the
average of the middle two numbers,
;with half up rounding.
; 
; Preconditions: Header string set, array is type DWORD, array size is positive 
int.
;
...
IDIV EBX
CMP  EDX, 0
; EAX is middle element in odd arrays, and first middle element in even 
arrays.
; x4, add array address to set EAX to proper element.
IMUL EAX, 4
ADD  EAX, [EBP + 24] ; array address
MOV  EDX, EAX ; EDX is set to EAX to retrieve next 
element if even.
MOV  EAX, [EAX]
JNE _even
JMP _writeNumber
...
MOV  EDX, [EDX]
ADD  EAX, EDX
IMUL EAX, 10 ; x10 numerator and divisor to capture 
fractional remainder as integer.
IMUL EBX, 10
CDQ
IDIV EBX
CMP  EDX, 5 ; if remainder is greater than 5, round 
up.
JL  _writeNumber
ADD  EAX, 1
_writeNumber:
...
; Calculate number of rows with 20 elements to a row and then the distance 
between elements in a row. (rows x element size)
MOV  EAX, ECX
MOV  EBX, 20
CDQ
IDIV EBX
...
MOV  EAX, EDX ; EAX is the number of columns with 
addlt. 4 added to offset between elements.
PUSH EAX
XOR  EDX, EDX ; EDX will track elements to line wrap at
20.
MOV  EDI, ESI ; EDI is now ESI, to mark start and 
increment by 4 each line wrap.
_displayLoop:
...
; Name: countList
;
; Description: Counts and fills an array of occurrences of each number in a 
provided array.
; 
; Preconditions: Min, max, and arraysize ints are positive, arrays are type DWORD.
;
; Postconditions: array2 will hold counts of occurrences of numbers in array1.  No 
register change.
;
; Receives: 
